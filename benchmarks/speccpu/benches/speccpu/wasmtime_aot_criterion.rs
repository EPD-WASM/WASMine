...
fn wasmtime_criterion_505(c: &mut Criterion) {
    util::wasmtime_aot_criterion(SPECCPU_505.clone(), c);
}

fn wasmtime_criterion_508(c: &mut Criterion) {
    util::wasmtime_aot_criterion(SPECCPU_508.clone(), c);
}

fn wasmtime_criterion_519(c: &mut Criterion) {
    util::wasmtime_aot_criterion(SPECCPU_519.clone(), c);
}

fn wasmtime_criterion_557(c: &mut Criterion) {
    util::wasmtime_aot_criterion(SPECCPU_557.clone(), c);
}
