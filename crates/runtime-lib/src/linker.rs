...
use wasi::{WasiContext, WASI_FUNCS};
            ...
            match &import.desc {
                ImportDesc::Func(type_idx) => {
                    ...
                    // Option 2 = WASI function
                        ...
                        let (boundary_func_ptr, func_type) =
                            WasiContext::get_func_by_name(&import.module, &import.name);
                        func = self.cluster.alloc_function(Function::from_wasi_func(
                            unsafe { std::mem::transmute(boundary_func_ptr) },
                            func_type,
                        ));
                        actual_functype = func.functype()
                    } else {
