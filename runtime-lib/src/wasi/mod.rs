...
pub(crate) fn collect_available_imports() -> HashMap<&'static str, RTFuncImport> {
    #[rustfmt::skip]
    return HashMap::from([
        ("wasi_snapshot_preview1.fd_filestat_get", RTFuncImport {name: "fd_filestat_get".into(), /* inputs: fd + return_ptr, outputs: errno */ function_type: FuncType(vec![I32, I32], vec![I32]), callable: functions::fd_filestat_get as RawFunctionPtr, execution_context: None},
    )]);
}
